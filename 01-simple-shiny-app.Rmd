...
1. add a sliderInput: 
   ...
   sliderInput(inputId = "n_bins",
               ...
               value = 50),
   ```
2. add a `plotOutput`, and the plot does not appear.
...
1. add `flog.info("some messages...")` inside your code and see when they are run.
...
2. use `output$plot_title` in `ggtitle` to set the title in the plot.
...
1. read a new dataset with `df <- read_csv("./data/marvel-vs-dc.csv")`
